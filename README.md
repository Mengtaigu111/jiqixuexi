...
先填写 `submission_info_template.md` 中的 GitHub 链接、作者贡献、研究领域和最终提交选择，再同步替换 README、报告草稿和 PDF 生成脚本中的占位符。

填入真实 GitHub 链接、作者贡献和研究领域后，运行：

```bash
PYTHON="conda run -n qwen3meld-run python" bash scripts/check_submission_ready.sh
```

如果正式信息仍未填写，脚本会失败并列出需要替换的文件和行号。

在正式信息尚未填写时，可只检查结果文件、PDF、图片和 ignore 规则：

```bash
PYTHON="conda run -n qwen3meld-run python" ALLOW_PLACEHOLDERS=1 SKIP_TESTS=1 bash scripts/check_submission_ready.sh
```

## 报告文件路径

- Markdown 草稿：`report/report_draft.md`
- PDF 报告：`report/report.pdf`
- 兼容文件名：`report/ML_household_power_report.pdf`
- 报告图片素材：`report/report_assets/`

报告应包含 GitHub 链接、作者贡献、研究领域、参考文献和工具使用说明。正式报告中的实验结果必须来自 `results/metrics/summary.csv` 和 `results/figures/`，不能使用 smoke 合成数据结果冒充正式结果。
...
[2] PyTorch Documentation. LSTM and TransformerEncoder. https://pytorch.org/docs/stable/nn.html
